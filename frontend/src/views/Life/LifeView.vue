...
const diaryForm = ref({ title: '', content: '', dimension: 'growth' as string, tags: '' })
...
function openNewDiary() {
  ...
  diaryForm.value = { title: '', content: '', dimension: 'growth', tags: '' }
  showDiaryEditor.value = true
}

function openEditDiary(d: Diary) {
  editingDiary.value = d
  diaryForm.value = { title: d.title || '', content: d.content || '', dimension: d.dimension || 'growth', tags: (d.tags || []).join(',') }
  showDiaryEditor.value = true
}
...
async function saveDiary() {
  try {
    ...
    if (editingDiary.value) {
      await lifeApi.updateDiary(editingDiary.value.id, { ...diaryForm.value, tags })
      toast.success('日记已更新')
    } else {
      await lifeApi.createDiary({ ...diaryForm.value, tags })
      toast.success('日记已创建')
    }
