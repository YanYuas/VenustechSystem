...
const form = ref({ name: '', description: '', color: '#7c5cff', status: 'active' as ProjectStatus, due_date: '', identity_id: '' })

function openCreate() {
  editing.value = null
  form.value = { name: '', description: '', color: '#7c5cff', status: 'active', due_date: '', identity_id: '' }
  loadIdentities()
  modalOpen.value = true
}

// 首页快速入口「新建项目」带 ?action=new 直达新建弹窗（R-02）
  ...
  <div class="projects">
    ...
    <BaseModal v-model="modalOpen" :title="editing ? '编辑项目' : '新建项目'" @confirm="onSubmit">
      <div class="projects__form">
        ...
        <div class="projects__form-half">
          <label class="projects__label">所属身份（可选）</label>
          <BaseSelect v-model="form.identity_id" :options="identityOptions(true)" placeholder="未归类" />
        </div>
      </div>
    </BaseModal>
  </div>
