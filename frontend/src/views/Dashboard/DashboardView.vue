...
const defaultOrder = ['focus', 'execution', 'projects', 'identities', 'resources', 'learning', 'recent', 'life']
...
const identityRows = computed(() => data.value?.identities.list ?? [])
const unassignedOpen = computed(() => data.value?.identities.unassigned_open ?? 0)
...
<template>
  <div class="dash">
    ...
    <!-- 今日执行（全宽） -->
    <BaseCard class="dash__execution" :style="{ order: cardOrder.indexOf('execution') }" scroll draggable="true" @dragstart="onDragStart($event, 'execution')" @dragover="onDragOver($event, 'execution')" @dragleave="onDragLeave" @drop="onDrop($event, 'execution')" :class="{ 'is-drag-over': dragOverCard === 'execution' }">
      <template #title>
        <div class="dash__card-head">
    ...
    <div class="dash__grid-3">
      ...
      <!-- 身份进度（三期 B）：一眼看出每条人生线推到哪了 -->
      <BaseCard :class="{ 'is-drag-over': dragOverCard === 'identities' }" :style="{ order: cardOrder.indexOf('identities') }" scroll draggable="true" @dragstart="onDragStart($event, 'identities')" @dragover="onDragOver($event, 'identities')" @dragleave="onDragLeave" @drop="onDrop($event, 'identities')">
        <template #title>
          <div class="dash__card-head">
            <h3 class="dash__card-title">🧭 身份进度</h3>
            <button class="dash__identity-link" type="button" @click="router.push('/experience')">
              经历时间线 →
            </button>
          </div>
        </template>
        ...
        <template v-else-if="identityRows.length">
          <div class="dash__identities">
            <button
              v-for="i in identityRows" :key="i.id"
              class="dash__identity" :class="{ 'is-archived': i.is_archived }"
              type="button"
              @click="router.push({ path: '/tasks', query: { identity_id: i.id } })"
            >
              <span class="dash__identity-dot" :style="{ background: colorVar(i.color_token) }" />
              <AppIcon :name="i.icon" :size="14" />
              <span class="dash__identity-name">{{ i.name }}</span>
              <span class="dash__identity-stats">
                <b :class="{ 'is-zero': i.open_tasks === 0 }">{{ i.open_tasks }}</b> 待办
                <template v-if="i.in_progress">· {{ i.in_progress }} 进行</template>
                <template v-if="i.completed_today">· 今日 +{{ i.completed_today }}</template>
              </span>
            </button>
            <p v-if="unassignedOpen" class="dash__identity-unassigned">
              另有 {{ unassignedOpen }} 个未归类任务 ——
              <router-link to="/identities">建立身份</router-link>后给任务挂上，线才看得清
            </p>
          </div>
        </template>
        <template v-else>
          <BaseEmpty title="还没有身份" description="身份是人生的分类轴：AI 研究、音乐、交易……各是一条线">
            <template #action><BaseButton variant="primary" size="sm" @click="router.push('/identities')">建立身份</BaseButton></template>
          </BaseEmpty>
        </template>
      </BaseCard>
...
.dash {
  ...
  // 身份进度（三期 B）
  &__identities { display: flex; flex-direction: column; gap: var(--space-2); }
  &__identity-link {
    margin-left: auto;
    border: none; background: transparent;
    font-size: var(--text-xs); color: var(--primary-ink);
    cursor: pointer;
  ...
  &__identity-stats {
    ...
    b { color: var(--primary-ink); font-weight: 600; &.is-zero { color: var(--text-low); } }
  }
  &__identity-unassigned {
