...
const TEMPLATE_KEY = 'venustech_ai_templates'
...
const builtinFiltered = computed(() => filteredTemplates.value.filter(t => t.builtin))
const customFiltered = computed(() => filteredTemplates.value.filter(t => !t.builtin))

// 自定义模板表单
const showCustomForm = ref(false)
const customForm = ref({ name: '', content: '', category: '自定义' })
const customCategories = ['自定义', '工作', '学习', '写作', '创意', '其他']
...
function saveCustomTemplates() {
  try { localStorage.setItem(TEMPLATE_KEY, JSON.stringify(customTemplates.value)) } catch { }
}
function addCustomTemplate() {
  if (!customForm.value.name.trim() || !customForm.value.content.trim()) {
  ...
  saveCustomTemplates()
  customForm.value = { name: '', content: '', category: '自定义' }
  showCustomForm.value = false
  toast.success('自定义模板已添加')
}
...
const PERSONA_KEY = 'venustech_ai_persona'

function selectPersona(p: Persona) {
  activePersona.value = p
  try { localStorage.setItem(PERSONA_KEY, p.id) } catch { }
...
try {
  ...
  if (savedPersona) {
    const found = personas.find(p => p.id === savedPersona)
...
// ---------- 多模型切换（M04 P1 F06）· 已localStorage持久化 ----------
interface AIModel {
...
<template>
  <div class="conv-view">
    ...
    <section class="conv-view__main">
      <!-- 人设状态栏 -->
      <div class="conv-view__persona-bar">
        <button class="conv-view__persona-btn" @click="showPersonaPicker = true">
          <span class="conv-view__persona-avatar">{{ activePersona.avatar }}</span>
          <span class="conv-view__persona-name">{{ activePersona.name }}</span>
          <span class="conv-view__persona-desc">{{ activePersona.description }}</span>
          <AppIcon name="chevron-down" :size="12" />
        </button>
        <button class="conv-view__model-btn" @click="showModelPicker = true">
          <span class="conv-view__model-icon">{{ currentModel.icon }}</span>
          <span class="conv-view__model-name">{{ currentModel.name }}</span>
          <AppIcon name="chevron-down" :size="12" />
        </button>
        <button class="conv-view__model-btn" @click="showProfilePanel = true">
          <AppIcon name="user" :size="14" />
          <span class="conv-view__model-name">画像</span>
        </button>
      </div>
      ...
      <!-- 提示词模板选择（M04 P0 F02 + 自定义模板） -->
      <BaseModal v-model="showTemplatePicker" title="选择提示词模板" :width="560">
        <div class="conv-tpl">
          <div class="conv-tpl__head">
            <input v-model="templateSearch" class="conv-tpl__search" placeholder="搜索模板…" />
            <BaseButton variant="primary" size="sm" @click="showCustomForm = !showCustomForm">
              <AppIcon name="plus" :size="14" /> {{ showCustomForm ? '取消' : '新建模板' }}
            </BaseButton>
          </div>

          <!-- 新建自定义模板表单 -->
          <div v-if="showCustomForm" class="conv-tpl__form">
            <input v-model="customForm.name" class="conv-tpl__input" placeholder="模板名称（如：周报生成）" />
            <select v-model="customForm.category" class="conv-tpl__select">
              <option v-for="c in customCategories" :key="c" :value="c">{{ c }}</option>
            </select>
            <textarea v-model="customForm.content" class="conv-tpl__textarea" placeholder="模板内容，可用 \n 换行" rows="3" />
            <BaseButton variant="primary" size="sm" @click="addCustomTemplate">保存模板</BaseButton>
          </div>

          <!-- 内置模板 -->
          <div v-if="builtinFiltered.length" class="conv-tpl__group">
            <span class="conv-tpl__group-label">内置模板</span>
            <div class="conv-tpl__grid">
              <button v-for="t in builtinFiltered" :key="t.id" class="conv-tpl__item" @click="applyTemplate(t)">
          ...
          <!-- 自定义模板 -->
          <div v-if="customFiltered.length" class="conv-tpl__group">
            <span class="conv-tpl__group-label">我的模板（{{ customTemplates.length }}）</span>
            <div class="conv-tpl__grid">
              <div v-for="t in customFiltered" :key="t.id" class="conv-tpl__item conv-tpl__item--custom">
                <button class="conv-tpl__item-btn" @click="applyTemplate(t)">
                  <span class="conv-tpl__icon"><AppIcon :name="t.icon" :size="16" /></span>
                  <span class="conv-tpl__name">{{ t.name }}</span>
                  <span class="conv-tpl__cat">{{ t.category }}</span>
                </button>
                <button class="conv-tpl__del" title="删除模板" @click.stop="deleteCustomTemplate(t.id)">
          ...
          <p v-if="!filteredTemplates.length && !showCustomForm" class="conv-tpl__empty">没有匹配的模板</p>
        </div>
      </BaseModal>
...
.conv-tpl {
  ...
  &__head { display: flex; gap: 8px; align-items: center; }
  &__head .conv-tpl__search { flex: 1; width: auto; }
  &__form {
    display: flex; flex-direction: column; gap: 8px;
    padding: 12px; background: var(--bg-inset); border-radius: var(--radius-md);
  }
  &__input, &__select, &__textarea {
    padding: 8px 12px; border: 1px solid var(--line); border-radius: var(--radius-sm);
    background: var(--bg-panel); color: var(--text-hi); font-size: var(--text-sm); font-family: inherit;
    &:focus { outline: none; border-color: var(--primary); }
  }
  &__textarea { resize: vertical; min-height: 60px; }
  &__group { display: flex; flex-direction: column; gap: 8px; }
  &__group-label { font-size: 11px; color: var(--text-low); font-weight: 600; padding-left: 4px; }
  &__item--custom {
    position: relative; display: flex; align-items: center; padding: 0 !important; overflow: hidden;
    .conv-tpl__item-btn {
    ...
    .conv-tpl__del {
      ...
      color: var(--text-low); cursor: pointer; margin-right: 8px;
      &:hover { background: var(--strawberry); color: white; }
    }
  }
}
.conv-persona {
