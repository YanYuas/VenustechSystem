...
// ---------- 身份轴（三期 B）：筛选 + 新建/编辑挂身份 ----------
const { loadIdentities, identityOptions } = useIdentity()
// BaseSelect 以空串表示「未选择」（显示占位符），因此筛选器用哨兵值：
// __all__ = 全部身份，__none__ = 未归类，其余 = 身份 id
...
function onFilterIdentity(v: string) {
  if (v === FILTER_ALL) {
    query.value.identity_id = undefined
    query.value.identity_unassigned = undefined
  } else if (v === FILTER_NONE) {
    query.value.identity_id = undefined
    query.value.identity_unassigned = true
  } else {
    query.value.identity_id = v
    query.value.identity_unassigned = undefined
  }
  query.value.page = 1
...
onMounted(() => {
  ...
  // 身份轴（三期 B）：Dashboard 身份卡片带 ?identity_id= 跳入
  const urlIdentity = route.query.identity_id as string
  if (urlIdentity) {
    query.value.identity_id = urlIdentity
    filterIdentityId.value = urlIdentity
  }
  fetchTasks().catch(() => { /* http 层已提示 */ })
})
...
async function onCreate() {
  ...
  try {
    await createTask({
      ...
      identity_id: newIdentityId.value || undefined,
    })
    toast.success('任务已创建')
...
// 首页快速入口「新建任务」带 ?action=new 直达新建弹窗（R-02）
...
async function saveDetail() {
  ...
  try {
    await updateTask(t.id, {
      ...
      identity_id: editForm.value.identity_id || undefined,
...
<template>
  <div class="tasks">
    ...
    <div class="tasks__tabs">
      ...
      <BaseSelect
        class="tasks__identity-filter"
        :model-value="filterIdentityId"
        :options="filterOptions"
        placeholder="身份"
        @change="onFilterIdentity"
      />
    </div>
    ...
    <BaseModal v-model="createOpen" title="新建任务" @confirm="onCreate">
      <div class="tasks__form">
        ...
        <BaseSelect v-model="newIdentityId" :options="identityOptions(true)" placeholder="所属身份（可选）" />
      </div>
    </BaseModal>
      ...
      <div v-if="detailTask" class="tasks__detail">
        ...
        <div class="tasks__detail-row">
          ...
          <div class="tasks__detail-section">
            ...
            <BaseSelect v-model="editForm.identity_id" :options="identityOptions()" placeholder="所属身份（可选）" />
...
.tasks {
  ...
  // 身份筛选（三期 B）：固定宽，贴在视图切换右侧
  &__identity-filter { width: 150px; flex: none; margin-left: var(--space-2); }
