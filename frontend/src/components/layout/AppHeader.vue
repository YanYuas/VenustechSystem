...
import type { GrowthState, NotificationType } from '@/types'
...
// S6-2 成长体系：等级徽章
//
// 放在顶栏而不是某个模块页里，是因为成长体系的卖点正是
// 「无论你在哪个模块做什么，都会在这里看到回响」。
//
// 刷新方式：后端的事件结算发生在服务端，前端无法订阅；
// 这里用 30 秒轻量轮询。相比引入全局状态管理或 SSE，
// 轮询的代价更低，且失败静默 —— 徽章是增强信息，
// 不该影响顶栏本身可用。
// ============================================================
const growth = ref<GrowthState | null>(null)
let growthTimer: number | undefined

async function loadGrowth() {
  try {
    growth.value = await growthApi.state()
  } catch {
    growth.value = null
  }
}
...
onMounted(() => {
  loadGrowth()
  growthTimer = window.setInterval(loadGrowth, 30000)
})

// 升级庆祝（S6-2 收尾）：徽章在轮询中发现 level 变高时，全站弹一次祝贺。
// only when 旧值存在 → 避免首次加载/刷新触发「假升级」。
watch(
  () => growth.value?.level,
  (nv, ov) => {
    if (nv && ov && nv > ov) {
      toast.success(`升级！LV${ov} → LV${nv}`, '持续积累，星光更亮了一点')
    }
  },
)

onUnmounted(() => {
  if (growthTimer !== undefined) window.clearInterval(growthTimer)
})

/** 点击进入成长页 —— 徽章是成长体系最自然的入口 */
function goGrowth() {
  ...
  <header class="header">
    ...
    <div class="header__search" :class="{ 'is-focus': searchFocused }">
      ...
      <div class="header__right">
        <!-- S6-2 成长体系：等级徽章（可点击进入成长页） -->
        <button
          v-if="growth"
          type="button"
          class="header__level"
          :title="`LV${growth.level} · ${growth.current}/${growth.need} EXP（累计 ${growth.exp}）· 点击查看详情`"
          @click="goGrowth"
        >
          <span class="header__level-badge">LV{{ growth.level }}</span>
          <span class="header__level-track">
            <span class="header__level-fill" :style="{ width: growth.percent + '%' }" />
          </span>
        </button>
...
.header {
  ...
  &__level {
    ...
    gap: var(--space-2);
    flex-shrink: 0;
    // 这是 <button>，需要抹掉浏览器默认样式，只保留内容
    padding: 0;
    border: none;
    background: transparent;
    cursor: pointer;
    &:hover .header__level-badge {
