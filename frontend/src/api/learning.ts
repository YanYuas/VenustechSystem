...
export const learningApi = {
  ...
  // ==================== S6-1 领域规则引擎（离线可用，不消耗 AI 额度）====================
  // 未配 API Key 时同样可用：走内置领域知识库（13 领域 / 90 条带达标标准的任务），
  // 未命中时退化为四阶段通用拆解。
  generatePlan(data: GeneratePlanParams) {
    return http.post<GeneratedPlan>('/learning/plans/generate', data)
  },
  applyPlan(data: ApplyPlanParams) {
    return http.post<ApplyPlanResult>('/learning/plans/generate/apply', data)
  },
