...
class DocumentService:
    ...
    def create_document(self, user_id: str, data: CreateDocumentRequest) -> DocumentOut:
        ...
        doc = self.doc_repo.create(
            ...
            identity_id=self._validated_identity_id(user_id, data.identity_id),
    ...
    def update_document(self, user_id: str, doc_id: str, data: UpdateDocumentRequest) -> DocumentOut:
        ...
        if "identity_id" in payload:
            # None = 解除关联；空串视同 None（前端下拉清空）
            doc.identity_id = self._validated_identity_id(user_id, payload["identity_id"])
        self.db.commit()
        self.db.refresh(doc)
        self._sync_backlinks(user_id, doc)
        if content_changed:
    ...
    def _validated_identity_id(self, user_id: str, identity_id: str | None) -> str | None:
        """身份轴写入前校验（三期 B）：None/空串=未归类；有值必须归属当前用户。"""
        if not identity_id:
            return None
