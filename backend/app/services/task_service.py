...
from app.core.exceptions import NotFoundException, TaskStateException, ValidationException
from app.models.base import utcnow
from app.models.task import FocusSession, Task
from app.repositories import IdentityRepository, SubtaskRepository, TaskRepository
...
class TaskService:
    ...
    def _ensure_identity_owned(self, user_id: str, identity_id: str) -> None:
    ...
    def list(
        ...
        tasks = self.repo.list_paginated(
            ...
            sort=sort, skip=skip, limit=page_size,
        )
        total = self.repo.count_paginated(
    ...
    def create(self, user_id: str, data: CreateTaskRequest) -> TaskOut:
        ...
        if data.identity_id is not None:
            self._ensure_identity_owned(user_id, data.identity_id)
    ...
    _UPDATABLE_FIELDS = {
        ...
        "estimated_minutes", "acceptance_criteria",
        "identity_id",
    }
    ...
    def update(self, user_id: str, task_id: str, data: UpdateTaskRequest) -> TaskOut:
        ...
        # 身份轴：写入前校验归属（identity_id 是应用层关联，无外键兜底）
        if payload.get("identity_id") is not None:
            self._ensure_identity_owned(user_id, payload["identity_id"])
        became_completed = False
