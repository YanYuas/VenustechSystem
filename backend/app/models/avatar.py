...
class AvatarMemory(UUIDMixin, TimestampMixin, SoftDeleteMixin, Base):
    """第二分身长期记忆"""
...
class AvatarInspiration(UUIDMixin, TimestampMixin, SoftDeleteMixin, Base):
    """第二分身灵感记录"""
...
class AvatarConfig(UUIDMixin, SoftDeleteMixin, Base):
    """第二分身配置（单例，每个用户一条）"""
