...
# ==================== 领域规则引擎生成计划（S6-1）====================
# 注意：这两条必须定义在 /plans/{plan_id} 之前，否则 "generate" 会被
# 当作 plan_id 匹配掉。

@router.post("/plans/generate", summary="从目标生成学习计划（离线可用，不消耗 AI 额度）")
async def generate_plan(
    data: GeneratePlanRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """仅预览，不落库。未配 API Key 时同样可用。"""
    return success(_svc(db).generate_plan_preview(data))


@router.post("/plans/generate/apply", summary="生成学习计划并落成任务")
async def apply_generated_plan(
    data: ApplyPlanRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """生成计划 → 写入 StudyPlan → 批量写入 Task（带预估时长与达标标准）。"""
    return success(_svc(db).apply_generated_plan(user.id, data))
