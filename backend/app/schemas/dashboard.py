...
# ---------- 完整 Dashboard 数据 ----------
class IdentityProgress(BaseModel):
    """单个身份的任务进度快照（三期 B）。"""
...
class IdentitiesSection(BaseModel):
    """身份进度区（按身份聚合任务 + 未归类计数）。

    注意字段名用 items 而非 list：字段名 `list` 会在 Pydantic 解析
    注解时把内建 list 遮蔽成 FieldInfo（实测 TypeError）。
    """

    items: list[IdentityProgress] = Field(default_factory=list)
...
class DashboardDataOut(BaseModel):
    ...
    # 身份进度（三期 B：按身份聚合任务，治「看不出推进哪条线」）
    identities: IdentitiesSection = Field(default_factory=IdentitiesSection)
