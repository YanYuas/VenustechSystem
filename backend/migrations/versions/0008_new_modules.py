...
def upgrade() -> None:
    ...
    # 修正（2026-09-15）：原为硬编码 print("... 15 tables created")，实际建表数因库而异
    # —— 全新库上 0001 的 create_all 已预建全部模型表，本迁移实际建 0 张。
    # 硬编码的假数字会误导排障（这正是 B-1 长期不可见的原因之一）。
    _after = {r[0] for r in conn.execute(
        sa.text("SELECT name FROM sqlite_master WHERE type='table'")
    ).fetchall()}
    _created = sorted(_after - _before)
    if _created:
        print(f"0008_new_modules: 新建 {len(_created)} 张表 -> {', '.join(_created)}")
    else:
        print("0008_new_modules: 涉及的表均已存在（0001 的 create_all 预建），本次未新建")
