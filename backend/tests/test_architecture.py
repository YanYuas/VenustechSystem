...
# ============================================================
# 免 pytest 运行入口
# 本机 venv 未安装 pytest（见《一期模块考校报告》E-01），导致本守护测试长期
# 无人执行、分层违规悄悄累积。此入口让它可以独立跑：
...
def _run_all() -> int:
    ...
    # 让 `python tests/test_architecture.py` 也能 import 到 app 包
    if str(BACKEND_DIR) not in sys.path:
        sys.path.insert(0, str(BACKEND_DIR))

    suite = TestArchitecture()
    checks = [n for n in dir(suite) if n.startswith("test_")]
    passed = failed = 0
    for name in checks:
        try:
            getattr(suite, name)()
