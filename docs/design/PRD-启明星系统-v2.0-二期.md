...
阶段六：外部交付物内核融合（第17-20周 · 详见 §3.1）
├── S6-1 领域知识库 + 学习计划生成引擎（P0）
├── S6-2 成长体系：经验值 / 等级 / 技能树（P0-P1）
├── S6-3 同步抽象层 + 本地适配器（P0，为三期云同步铺路）
├── S6-4 结构化经历 + 能力溯源（P1，须先做概念归并）
├── S6-5 单文件 HTML 报告导出引擎（P2）
├── S6-6 AI HOT 信息源插件（P2）
└── S6-7 形象合成机制（P3，已降级）
```

#### 3.1 阶段六任务清单（外部交付物内核融合）

> 来源报告（均已归档）：
> - [`docs/management/三交付物融合方案-2026-09-15.md`](../management/三交付物融合方案-2026-09-15.md)——B 生活工作台 / C 日报仪表盘的全部内核
> - [`docs/management/地球Online内核融入效果评估-2026-09-15.md`](../management/地球Online内核融入效果评估-2026-09-15.md)——地球Online 内核的逐块效果评估
>
> 三条铁律：**内核优先于功能**（原生 JS 不可直搬，须重写为 Vue3+TS）／**数据主权不迁移**（融合同步算法而非同步依赖）／**视觉不越界**（只取内核，不动 4 套主题）。

| 任务 | 内容 | 优先级 | 主要风险 |
|------|------|--------|----------|
| **S6-1** 领域知识库 + 计划生成引擎 | 迁移 `DOMAIN_LIB`（13 领域 / 52 能力单元 / 90 条带达标标准的任务）+ `aiGeneratePlan()` + `genericUnits()` 兜底；接入无 API Key 降级路径 | **P0** | `Task` 表缺「预估时长」「达标标准」两列，需 schema 变更 |
| **S6-2** 成长体系 | EXP 规则 + 等级递推（`80+lv*20*1.12^lv`，LV30 累计 107,069）+ `SKILL_CATS` 22 分类技能树；**全部通过 `event_bus` 订阅挂载，不改现有模块代码** | **P0-P1** | 经验语义稀释，需重标权重（沉淀类 ≫ 处理类） |
| **S6-3** 同步抽象层 | `core/sync.py`（canon / 三态 diff / `recoverOnly`）+ `useSyncEngine.ts` + 本地适配器；**本期不接云**，云归三期 | **P0** | 无真正冲突解决，须预留字段级 LWW 接口 |
| **S6-4** 结构化经历 + 溯源 | 经历结构化四要素 + `sourceExpIds` 能力溯源 | **P1** | **与 Diary / Review / AvatarMemory / ProjectMemory 重叠，须先做概念归并** |
| **S6-5** 报告导出引擎 | 「数据 → 单文件 HTML」脚本化生成 + 硬性验收清单（零外部资源 / 无 ISO 残留 / 连续编号 / 响应式） | P2 | 低 |
| **S6-6** AI HOT 信息源插件 | 日报 API 抓取 → Resource 收集箱 → 经 S6-5 导出看板；同时验证插件架构可用性 | P2 | 低 |
| **S6-7** 形象合成机制 | 仅保留 canvas「背景 × 角色」合成逻辑，**不引入 30MB 国风素材** | P3 | 素材与 4 套主题风格冲突；违反 token 唯一来源约束 |

**已评估为不做的项**（避免重复评估）：

| 项 | 不做原因 |
|---|---|
| 移植地球Online 界面 | 原生 JS 单页站 + 独立设计规范，与 Vue3+TS 严格模式及 4 套主题冲突 |
| 直接接入资料库云端同步 | PRD v2.0 §1.3 已明确「不做云端同步」；S6-3 只做抽象层 |
| 引入地球Online 的 DBSync | 整对象覆盖式，被 S6-3 的 diff-sync 完全覆盖 |

#### 3.2 阶段六前置伏笔（2026-09-15 已铺设）

阶段六动工前，已先把「护栏」和「可测量基线」铺好，避免重蹈一期踩过的坑。

| 伏笔 | 对应任务 | 内容 |
|------|----------|------|
| **迁移幂等守卫 + 模板** | S6-1 / S6-3 / S6-4 的所有新迁移 | `app/migration_helpers.py` 提供 `table_exists`/`index_exists`/`column_exists`；`migrations/script.py.mako` 模板默认引入并写明原因。**新增迁移不会再因漏守卫而中断全新库**（B-1 复发防护） |
| **可执行的验证入口** | 全部 | `scripts/check_all.py` 一条命令跑齐架构守护 + 后端冒烟 + 前端类型检查，**零第三方依赖**（本机 venv 无 pytest）。解决「测试写了却没跑」的根因 |
| **同步就绪性审计** | **S6-3** | `backend/scripts/audit_sync_readiness.py` 逐表审计 `updated_at` / `deleted_at`。**实测基线：39 张表中仅 7 张就绪**（conversations / documents / folders / projects / reviews / tasks / users），**32 张需在 S6-3 前补齐** —— 5 张两者都缺（habit_checkins / mood_logs / sop_versions / study_time_logs / workflow_applications），27 张缺软删除 |
| **设计令牌债务台账** | 视觉相关改动 | `scripts/audit_hardcoded_colors.py` 实测组件内**硬编码颜色 171 处 / 16 个文件**（约束原为「组件内 `#` 零命中」）。支持 `--max N` 棘轮阈值。**注意：58 种色值在令牌体系中并不存在**，属组件自行引入的调色板，需设计决策而非机械替换 |
| **分支隔离能力** | 全部 | `scripts/dev.ps1 -DataDir <目录>` 让分支使用独立数据库，避免分支上的新迁移污染主库（详见《分支开发指南》） |

**S6-3 的真实工作量由此确定**：先补齐 32 张表的同步字段（含新的 Alembic 迁移），再谈 diff-sync 算法落地。
这是此前评估中「一条融合线」被低估的部分。
...
| v2.0-draft.1 | 2026-09-15 | 新增**阶段六：外部交付物内核融合**（§3 路线图 + §3.1 七项任务清单 S6-1~S6-7）；归档两份来源报告至 `docs/management/` |
