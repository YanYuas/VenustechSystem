...
| 编号 | 方向 | 涉及子模块 | 对应缺口 |
|------|------|------------|----------|
| P0-1 | 工作区搜索防抖 + 分页加载 | 工作区 | G1 / G2 |
| P0-2 | 保险箱移动端表单适配（单列堆叠） | 保险箱 | G4 |
| P0-3 | 保险箱「更换主密码」前端入口 | 保险箱 | G5 |
| P0-4 | 移动端整体 375px 适配打磨（四个子模块） | 全部 | G3 / G4 / G7 |
| P0-5 | 语音识别率优化：Web Speech 通道开启 interimResults 实时上屏 | AI 助理 | G9 |
| P0-6 | 离线队列基础设施（写操作本地排队 + 联网自动同步） | 全部写操作 | G10 / G14 |

### 3.2 P1（本期尽量交付，体验增量明显）

| 编号 | 方向 | 涉及子模块 | 对应缺口 |
|------|------|------------|----------|
| P1-1 | 保险箱凭据搜索/筛选（按名称/分类/身份） | 保险箱 | G6 |
| P1-2 | AI 资讯手动刷新按钮 + 下拉刷新 | AI 资讯 | G8 |
| P1-3 | 助理建议条目可删除（× 按钮） | AI 助理 | G11 |
| P1-4 | 工作区扫描异步化 + 进度反馈 | 工作区 | G12 |
| P1-5 | 保险箱 SSH 配置连通性测试按钮 | 保险箱 | G13 |
| P1-6 | AI 资讯移动端 Tab 横滑容器 | AI 资讯 | G7 |
